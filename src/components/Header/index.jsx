...
export default function Header() {
  ...
  return (
    <header
      ref={Href}
      className={` dark:text-zinc-50 absolute top-0 w-full z-[1] ${outfit.className} p-6 flex justify-between items-center `}
    >
      <div className="container flex justify-between items-center">
        <Magnetic>
          <div className="flex items-center font-semibold gap-2 group">
            <Link href="/" className="text-xl">
              <svg
                className="group-hover:rotate-180 group-hover:fill-red-400 transition-all ease-linear duration-300"
              ...
              >
                <path
                  className="group-hover:fill-red-500 dark:fill-zinc-50 transition-colors duration-300"
                  fill="currentColor"
            ...
            <h1>
              <Link
                href="/"
                className="relative text-xl dark:text-zinc-50 max-[500px]:hidden"
              >
                Gowtham S
              </Link>
            </h1>
        ...
        <nav className="hidden md:flex justify-center items-center gap-4 font-normal">
          <Rounded>
            <a href="/Gowtham_S.pdf" download="Gowtham_S.pdf" className=" flex items-center gap-2 z-[1]">Resume
              <Icon className="text-dark dark:text-light z-[1] text-xl" icon="line-md:download-outline-loop" />
            </a>
          </Rounded>
          <CustomLinks
            href="/projects"
        ...
        <div
          ...
          className={`fixed flex flex-col scale-0 gap-1 justify-center items-center m-5 p-2 right-0 md:right-10 md:h-14 md:w-14 h-10 w-10 cursor-pointer drop-shadow-xl shadow-2xl  rounded-full bg-gray-900 dark:bg-dark z-[80] group border-2 border-red-500 ${isClicked ? "bg-red-500 dark:bg-red-500  " : ""
            } overflow-hidden `}
        >
          <Rounded
            className={`h-20 w-20 rounded-full flex flex-col items-center justify-center gap-1 md:p-4 p-6  `}
          >
            <span
              className={`relative  z-30 h-[1px] w-[50%] bg-zinc-50 dark:bg-light ${isClicked ? "rotate-45 top-[2px]" : ""
                } transition-transform ease-linear duration-200`}
            ></span>
            <span
              className={`relative z-30 h-[1px] w-[50%] bg-zinc-50 dark:bg-light ${isClicked ? "-rotate-45 -top-[3px]" : ""
                } transition-transform ease-linear duration-200`}
            ></span>
          </Rounded>
        </div>
        ...
        <AnimatePresence mode="wait">
          {isClicked && (
            <Nav setIsClicked={setIsClicked} mode={mode} setMode={setMode} />
          )}
        </AnimatePresence>
      </div>
    </header>
