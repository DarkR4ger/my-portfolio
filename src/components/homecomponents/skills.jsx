...
function Box({ src, name, className, index }) {
  ...
  return (
    <motion.div ref={desc} className=" relative group flex flex-col justify-center items-center cursor-pointer bg-light dark:bg-dark h-32 p-4 sm:p-14 md:p-24 drop-shadow-xl shadow-2xl hover:shadow-red-500 tranistion duration-200  gap-4 overflow-hidden">
      ...
      <motion.p custom={index} variants={slideUp} initial='initial' animate={isInView ? 'open' : 'close'} className={`text-md md:text-xl group-hover:text-red-500 transition-colors duration-200  pt-2 md:pt-8 whitespace-nowrap font-medium uppercase ${className}`}>{name}</motion.p>
    </motion.div>
  )
}
...
export default function Skill() {
  ...
  return (
    <div className="h-screen  flex flex-col items-center">
      ...
      <div className=" relative mt-20 grid grid-cols-2 sm:grid-cols-3  xl:grid-cols-5 gap-5 items-center">
